...
def minutes_since_session_open() -> Optional[float]:
    ...
    logger.info("Requesting Tradier session window to calculate minutes since session open.")
    ...
    if session_window is None:
        logger.info("Tradier session window lookup found no open market session for %s.", session_now.date())
        return None
    open_time, _ = session_window
    minutes_since_open = (session_now - open_time).total_seconds() / 60.0
    logger.info("Tradier session window indicates %.2f minutes since session open.", minutes_since_open)
